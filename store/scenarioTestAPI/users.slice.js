...
const usersSlice = createSlice({
  ...
  extraReducers: {
    ...
    [api_v1_users_read.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [api_v1_users_read.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = [
    ...
    [api_v1_users_read.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error
        state.api.loading = "idle"
      }
    ...
    [api_v1_users_delete.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [api_v1_users_delete.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
    ...
    [api_v1_users_delete.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error
        state.api.loading = "idle"
      }
